# Scaffolded by scripts/add-pattern01-unit.sh for unit_key "retail_dev".
# Workspace/catalog/role data comes entirely from
# var.pattern01_units["retail_dev"] (pattern01_units.auto.tfvars) -- never
...
# variables.tf).
module "pattern01_unit_retail_dev" {
  ...
  workspace               = var.pattern01_units["retail_dev"].workspace
  catalogs                = var.pattern01_units["retail_dev"].catalogs
